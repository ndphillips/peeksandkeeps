...
\begin{center}
<<echo = F, fig.width = 3, fig.height = 3>>=

par(mar = rep(0, 4))
plot(1, xlim = c(1, 4), ylim = c(1, 4), 
     xaxt = "n", yaxt = "n", ylab = "", 
     xlab = "", type = "n", bty = "n")

text(rep(1, 4), 4:1, labels = c("State", "First", "Second", "Third"), cex = c(1.5, rep(1, 3)), adj = 0)
text(rep(2, 4), 4:1, labels = c("SAU", "1. Selection", "2. Action", "3. Update"), cex = c(1.5, rep(1, 3)), adj = 0)
text(rep(3, 4), 4:1, labels = c("ASU", "1. Action", "2. Selection", "3. Update"), cex = c(1.5, rep(1, 3)), adj = 0)

arrows(rep(1.1:3.1, times = 1, each = 2), 
       rep(3:2, times = 3) - .2,
       rep(1.1:3.1, times = 1, each = 2),
       rep(2:1, times = 3) + .15,
       length = .2
       )
...
<<echo = F, fig.width = 4, fig.height = 4, message = F>>=
require("circlize")
sm.select.fun <- function(trial = 10, expectations = c(2, 1), total.trials = 100, c = 1, kink = NA) {

  if(is.na(kink)) {kink <- total.trials * .5}
  
  output <- exp((trial / kink) ^ c * expectations) / sum(exp((trial / kink) ^ c * expectations))
  
return(output)

}

c.seq <- seq(0, 4, length.out = 4)
col.fun <- colorRamp2(breaks = c(min(c.seq), max(c.seq)), colors = c("blue", "red"), transparency = .3)

plot(1, type = "n", xlab = "trial", 
     ylab = "Select A Prob", xlim = c(0, 100), ylim = c(.4, 1),
    main = "Softmax Selection Rule")

for(c.i in c.seq) {

  
select.a.probs <- unlist(lapply(1:100, function(x) {sm.select.fun(trial = x, 
                    expectations = c(2, 1), 
                    c = c.i,
                    total.trials = 100)[1]}))
...
legend("bottomright", legend = paste("c = ", round(c.seq, 2)),
       col = col.fun(c.seq),
       lty = 1,
       cex = .5
       )

mtext("E(A) = 2, E(B) = 1", side = 3)
...
<<echo = F, results = 'hide'>>=
pdf("keepneg.pdf", width = 6, height = 6)

keep.neg.fun <- function(pneg, neg.tol = .5, neg.sens = .1) {

return(1 / (1 + exp(neg.sens * (pneg - neg.tol))))

}

gamma.seq <- seq(-1, 2, length.out = 5)
alpha.seq <- c(.1, 2, 4, 10)
require("circlize")
...
for(alpha.i in alpha.seq) {

plot(1, type = "n", xlab = "pneg", 
     ylab = "Keep Prob", xlim = c(0, 1), ylim = c(0, 1),
    main = paste("Alpha = ", alpha.i, sep = ""))

for(gamma.i in gamma.seq) {

col.fun <- colorRamp2(breaks = c(-1, 2), colors = c("blue", "red"),
transparency = .3 )
  
lines(seq(0, 1, .1), 
      keep.neg.fun(seq(0, 1, .1), 
      neg.tol = gamma.i, 
      neg.sens = alpha.i),
      col = col.fun(gamma.i))
...
legend("right", legend = paste("gamma = ", gamma.seq),
       col = col.fun(gamma.seq),
       lty = 1,
       cex = .5, bg = "white"
       )
...
<<echo = F, results = 'hide'>>=
pdf("keepexp.pdf", width = 9, height = 3)

keep.exp.fun <- function(exp, gamma = 1, alpha) {

return(1 / (1 + exp(-(exp - alpha) * gamma)))

}

gamma.seq <- seq(0, 5, length.out = 5)
alpha.seq <- c(-2, 0, 2)
...
plot(1, type = "n", xlab = "Expectation", 
     ylab = "Keep Prob", xlim = c(-5, 5), ylim = c(0, 1),
    main = paste("Alpha = ", alpha.i, sep = ""))

for(gamma.i in gamma.seq) {

col.fun <- colorRamp2(breaks = c(min(gamma.seq), max(gamma.seq)), 
                      colors = c("blue", "red"),
                      transparency = .3 )
  
lines(seq(-5, 5, .1), 
      keep.exp.fun(seq(-5, 5, .1), 
      gamma = gamma.i, alpha = alpha.i),
      col = col.fun(gamma.i))
...
<<echo = F, fig.width = 9, fig.height = 3>>=

switch.to.explore.fun <- function(
                                  trial, # Current trial
                                  gamma = 1, # Bias towards exploitation state [-Inf, Inf]
                                  beta = 1, # Trials since previous exploitation state
                                  n.trials = 100, # Total Trials,
                                  kink = 50 # Kink in function (location of .50)
                                  ) {
...
for (beta.i in floor(seq(0, 50, length.out = 3))) {

plot(5:100, ylim = c(0, 1), 
     type = "n", 
     main = paste("Trials since previous explore = ", beta.i, sep = ""),
     xlab = "Current Trial",
     ylab = "p(explore)"
     )

bias.seq <- seq(-10, 10, 5)
col.vec <- gray(seq(.1, .9, length.out = length(bias.seq)))

for (i in 1:length(bias.seq)) {

lines((floor(beta.i)):100, switch.to.explore.fun((floor(beta.i)):100, gamma = bias.seq[i], beta = beta.i, kink = 40), col = col.vec[i])
  
  }

legend("right", 
       legend = paste("gamma = ", bias.seq, sep = ""),
       lty = 1,
       col = col.vec
       )
...
kg.sim <- function(
  ...
  for (i in 1:nrow(result)) {
    ...
    if(sampling.method == "sequential") {
      ...
      outcome.i <- selected.gamble.pdf[[result$trial[i]]]
...
kg.sim(gamble.pdf = list(rnorm(200, -10, 30),
                        c(rnorm(100, 0, 30), rnorm(100, 0, 10)),
                        c(rnorm(100, 10, 10), rnorm(100, 10, 30))
                        ),
       action.method = "trial",
       action.pars = -2,
       plot.lag = 20,
