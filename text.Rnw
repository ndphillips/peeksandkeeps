...
peeks.vec <- seq(0, n.trials, 5) # Number of peeks people could take

# Create the environments

environments.ls <- list()

environments.ls[[1]] <- data.frame(
                              a = rnorm(1000, mean = -5, sd = 5), 
                              b = rnorm(1000, mean = 0, sd = 5), 
                              c = rnorm(1000, mean = 5, sd = 5))

environments.ls[[2]] <- data.frame(
                              a = rnorm(10000, mean = -5, sd = 30), 
                              b = rnorm(10000, mean = 0, sd = 30), 
                              c = rnorm(10000, mean = 5, sd = 30))

environments.ls[[3]] <- data.frame(
                              a = rnorm(10000, mean = -5, sd = 5),
                              b = rnorm(10000, mean = -5, sd = 5), 
                              c = rnorm(10000, mean = 5, sd = 5))

environments.ls[[4]] <- data.frame(
                              a = rnorm(10000, mean = -5, sd = 30),
                              b = rnorm(10000, mean = -5, sd = 30), 
                              c = rnorm(10000, mean = 5, sd = 30))

environments.ls[[5]] <- data.frame(
                              a = c(rnorm(9500, mean = 3, sd = 5),
                                    rnorm(500, mean = -100, sd = 5)),
                              b = rnorm(10000, mean = 1, sd = 5),
                              c = rnorm(10000, mean = 0, sd = 5))

environments.ls[[6]] <- data.frame(
                              a = c(rnorm(9500, mean = -3, sd = 5),
                                    rnorm(500, mean = 100, sd = 5)),
                              b = rnorm(10000, mean = 1, sd = 5),
                              c = rnorm(10000, mean = 0, sd = 5))
...
peeks.sim <- expand.grid(
                         ...
                         select.strategy = c("softmax"),  # egreedy or softmax
                         update.method = c("rl.3"),  # mean or rl.3
                         environment = 1:length(environments.ls),
                         sim = 1:n.sim,
                         total.points = NA, p.select.best = NA
                         )
...
peek.sim.fun <- function(i) {
  ...
  soft.max <- function(exp.vec, trial, sens = 1) {
    ...
    return(exp((trial / 10) ^ sens * exp.vec) / sum(exp((trial / 10) ^ sens * exp.vec)))
    
  }
  e.greedy <- function(exp.vec, trial, e = .05) {
  ...
  select.strategy.i <- peeks.sim$select.strategy[i]
  update.method.i <- peeks.sim$update.method[i]
  environment.i <- peeks.sim$environment[i]
  
  
  environment.outcomes <- environments.ls[[environment.i]]
    n.options <- ncol(environment.outcomes)
  environment.means <- unlist(lapply(1:n.options, function(x) {mean(environment.outcomes[,x])}))  
  best.option <- which(environment.means == max(environment.means))
  

  
  agent.result <- data.frame("trial" = 1:n.trials.i, 
                             selection = NA, 
                             action = NA,
                             outcome = NA,
                             reward = NA,
  ...
  for(trial.i in 1:n.trials.i) {
    ...
    if(current.action == "keep") {
      ...
      if(select.strategy.i == "egreedy") {
        
        select.probs <- e.greedy(current.expectations, 
                                 trial = trial.i, 
                                 e = .1)
...
# Get final output  
  ...
  p.select.best <- mean(agent.result$selection == best.option, na.rm = T)
  
  return(c(total.points, p.select.best))
  
}

sfInit(parallel = T, cpus = 16, slaveOutfile = "slaveupdate.txt")
sfExport("peeks.sim")
sfLibrary("snowfall", character.only = TRUE)
sfExport("environments.ls")
...
# Append to peeks.sim
peeks.sim[, 8:9] <- cluster.result.r

# Write results
saveRDS(environments.ls, file = paste("/simulations/sim_environments ", substr(date(), 1, 10), ".ls", sep = ""))

write.table(peeks.sim, file = paste("/simulations/sim_result ", substr(date(), 1, 10), ".txt", sep = ""), sep = "\t")
...
environments.ls <- readRDS("/simulations/sim_environments Mon Jul 27.ls")
peeks.sim <- read.table("/simulations/sim_result Mon Jul 27.txt", sep = "\t")


par(mar = c(5, 4, 4, 1))
par(mfcol = c(2, length(environments.ls)))

for(i in 1:length(environments.ls)) {
  
  env.df <- environments.ls[[i]]
  peeks.df <- subset(peeks.sim, environment == i & select.strategy == "softmax" & action.strategy == "trial" & update.method == "mean")
  
  
# Draw distributions
{
    n.options <- ncol(env.df)
    
    col.vec <- brewer.pal(7, "Set3")[4:(4 + n.options - 1)]
    
    min.x <- -50
    max.x <- 50
    
#     min.x <- min(env.df[,1:n.options])
#     max.x <- max(env.df[,1:n.options])
    
    dens.ls <- vector("list", n.options)
    
    min.y <- Inf
    max.y <- 0
    for(i in 1:n.options) {
      
      dens.ls[[i]] <- density(env.df[,i])
      
      min.y <- min(c(min.y, min(density(env.df[,i])$y)))
      max.y <- max(c(max.y, max(density(env.df[,i])$y)))
    }
    
    plot(1, xlim = c(min.x, max.x), ylim = c(min.y, max.y), xlab = "x", ylab = "density", type = "n")
    
    for (i in 1:n.options) {
      
      lines(dens.ls[[i]]$x, dens.ls[[i]]$y, col = col.vec[i])
      
      abline(v = mean(env.df[,i]), col = col.vec[i], lwd = 4)
      
    }
    
    legend("topright",  
           unlist(lapply(1:n.options, function(x) {
             paste("mean = ", round(mean(env.df[,x]), 0), ", sd = ", round(sd(env.df[,x]), 0), sep = "")})),
           lty = 1,
           col = col.vec)
  }
...
{
plot(1, xlim = c(min(peeks.df$n.peeks), 100),
     ylim = c(0, 500), type = "n")
...
selswitch.cond.bmod <- MCMCglmm(selectionswitch.mean ~ condition.difficulty + condition.stability + condition.mode, 
                                data = pk.p, nitt = mcmc.samples, burnin = mcmc.burnin)

selswitch.demo.bmod <-MCMCglmm(selectionswitch.mean ~ barratt.all + 
                                 reg.sch.all + max.nen.all, 
                                data = subset(pk.p.r, demo.valid), nitt = mcmc.samples, burnin = mcmc.burnin)

selswitch.bmod <- MCMCglmm(selectionswitch.mean ~  barratt.all + reg.sch.all + max.nen.all,
                               data = subset(pk.p, 
                                             subset = demo.valid), nitt = mcmc.samples, burnin = mcmc.burnin)
...
peeks.cond.bmod <- MCMCglmm(n.peeks ~ condition.difficulty * condition.stability, 
                        data = subset(pk.p, subset = condition.mode == "peek"), nitt = mcmc.samples, burnin = mcmc.burnin)


peeks.demo.bmod <- MCMCglmm(n.peeks ~ sex + age + barratt.all + reg.sch.all + max.nen.all, 
                            data = subset(pk.p, 
                                          subset = condition.mode == "peek" & demo.valid), nitt = mcmc.samples, burnin = mcmc.burnin)
...
peeks.cond.trial.bmod <- MCMCglmm(peek.bin ~ trial.c + condition.difficulty + condition.stability, 
                                  ...
                                  family = "categorical", nitt = mcmc.samples, burnin = mcmc.burnin)
...
rt.cond.bmod <- MCMCglmm(rt.l.median ~ condition.difficulty + condition.stability + condition.mode, 
                         data = subset(pk.p, subset = is.finite(rt.l.median)), nitt = mcmc.samples, burnin = mcmc.burnin)
    # No effect of conditions on overall rt

rt.demo.bmod <- MCMCglmm(rt.l.median ~ sex + age + barratt.all + reg.sch.all + max.nen.all,
                         data = subset(pk.p, subset = is.finite(rt.l.median) & demo.valid), nitt = mcmc.samples, burnin = mcmc.burnin)
    # Maximizers are slower

rt.cond.trial.bmod <- MCMCglmm(rt.l ~ condition.difficulty + condition.stability + mode + trial.c, 
                         random = ~workerid,
                         data = subset(pk.r, subset = condition.mode == "peek" & is.finite(rt.l)), nitt = mcmc.samples, burnin = mcmc.burnin)
...
pointsfh.cond.bmod <- MCMCglmm(total.points.fh ~ condition.mode + condition.difficulty + condition.stability, data = pk.p, nitt = mcmc.samples, burnin = mcmc.burnin verbose = F)

pointssh.cond.bmod <- MCMCglmm(total.points.sh ~ condition.mode + condition.difficulty + condition.stability, data = pk.p, nitt = mcmc.samples, burnin = mcmc.burnin, verbose = F)

pointsall.cond.bmod <- MCMCglmm(total.points ~ condition.mode + condition.difficulty + condition.stability, data = pk.p, nitt = mcmc.samples, burnin = mcmc.burnin, verbose = F)
...
pointsall.demo.bmod <- MCMCglmm(total.points ~ sex + age + barratt.all + reg.sch.all + max.nen.all, 
                                         data = subset(pk.p, 
                                                       age %in% 16:100 & demo.valid), nitt = mcmc.samples, burnin = mcmc.burnin)
...
peeks.demo.bmod <- MCMCglmm(n.peeks ~ sex + age + barratt.all + reg.sch.all + max.nen.all, 
                            ...
                            nitt = mcmc.samples, burnin = mcmc.burnin,
                            verbose = F)
...
participant.switch.bmod <- MCMCglmm(selectionswitch.mean ~ modeswitch.mean, 
                            ...
                            verbose = F, nitt = mcmc.samples, burnin = mcmc.burnin)
@
...
peeks.points.mod <- MCMCglmm(total.points ~ n.peeks * condition.stability * condition.difficulty,
                 data = subset(pk.p.r, condition.mode == "peek"), verbose = F, nitt = mcmc.samples, burnin = mcmc.burnin)
@
...
selswitch.bmod <- MCMCglmm(selectionswitch.mean ~  barratt.all + reg.sch.all + max.nen.all,
                               data = subset(pk.p, 
                                             subset = demo.valid), verbose = F, nitt = mcmc.samples, burnin = mcmc.burnin)

modeswitch.bmod <- MCMCglmm(modeswitch.mean ~  barratt.all + reg.sch.all + max.nen.all,
                               data = subset(pk.p, 
                                             subset = demo.valid & condition.mode == "peek"), verbose = F, nitt = mcmc.samples, burnin = mcmc.burnin)
